...
file_path = "./data"
anime = pd.read_csv(os.path.join(file_path, "anime.csv"))
ratings = pd.read_csv(os.path.join(file_path, "rating.csv"))
...
try:
    with open("./backend/als_model.pkl", "wb") as f:
        pickle.dump(model, f)
    with open("./backend/anime_id_to_index.pkl", "wb") as f:
        pickle.dump(anime_id_to_index, f)
    with open("./backend/user_id_to_index.pkl", "wb") as f:
